...
class MarcacaoForm(forms.ModelForm):
    ...
    class Meta:
        ...
        widgets = {'motivo': forms.RadioSelect, 'protese': forms.RadioSelect}
...
class AgendaForm(forms.ModelForm):
    ...
    def __init__(self, *args, **kwargs):
        ...
        with different_locale('pt_BR.UTF-8'):
            ...
            for semanas in TextCalendar().monthdatescalendar(self.ano, self.mes):

                for dias in semanas:

                    if dias.month == self.mes and dias.weekday() not in [5, 6]:

                        vaga_dia.append((dias.day, '{:02d} - {}'.format(
